...
BINARY_FILE_NAME=conditions-number
...
test: vet
	...
	cat ${COVERAGE_FILE_NAME} | grep -v _mock.go | grep -v _logging.go | grep -v .pb.go > ${COVERAGE_FILE_NAME}.tmp
...
run: docker
	docker run \
		-d \
		--name awakari-conditions-number \
		-p 50051:50051 \
		--expose 50051 \
		awakari/conditions-number
